...
placeOrderBtn.addEventListener('click', () => {
    ...
    // Update total bill back to base amount without extra charge
    updateTotalBill();
});
...
placeOrderButton.addEventListener('click', async function() {
    ...
    const response = await fetch(${backendUrl}/place-order, {
        ...
        body: JSON.stringify({ items, totalAmount: parseFloat(totalBillElement.textContent) }),
    });
    ...
    if (response.ok) {
        ...
        orderDetails.innerHTML = items.map(item => <p>${item.name} x ${item.quantity} = ₹${(item.price * item.quantity).toFixed(2)}</p>).join('');
        totalBillElement.textContent = parseFloat(totalBillElement.textContent).toFixed(2);
    } else {
        alert('Error placing order: ' + data.message);
    }
});  
